		       ...
		       const char *bin, struct process_metadata *metadata)
{
	...
	for (i = 0; i < elf->header.e_phnum; ++i) {
		...
		if (elf->p_headers[i].p_type == PT_LOAD) {
			...
			seg_map_sz =
				ROUND_UP(p_vaddr + elf->p_headers[i].p_memsz,
					 PAGE_SIZE) -
			...
			// printk("p_vaddr: %lx, p_vaddr aligned: %lx, seg_sz: %lx, seg_map_sz: %lx, pmo start: %lx, p_offset: %lx\n",
			//        p_vaddr, ROUND_DOWN(p_vaddr, PAGE_SIZE), seg_sz,
			//        seg_map_sz, pmo->start,
			...
			memcpy((void *)(pmo_start_va + begin_pad),
			       (void *)((u64)bin + elf->p_headers[i].p_offset),
			       seg_sz);
			u64 bss_size = seg_map_sz - begin_pad - seg_sz;
			if (bss_size > 0)
				memset((void *)(pmo_start_va + begin_pad +
						seg_sz),
				       0, bss_size);
