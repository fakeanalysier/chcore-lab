...
{
	return syscall(SYS_unmap_pmo, process_cap, pmo_cap, addr, 0, 0, 0, 0, 0,
		       0);
}
...
{
	return syscall(SYS_register_client, server_cap, vm_config_ptr, 0, 0, 0,
		       0, 0, 0, 0);
}
...
{
	return syscall(SYS_cap_copy_to, dest_process_cap, src_slot_id, 0, 0, 0,
		       0, 0, 0, 0);
}

int usys_cap_copy_from(u64 src_process_cap, u64 src_slot_id)
{
	return syscall(SYS_cap_copy_from, src_process_cap, src_slot_id, 0, 0, 0,
		       0, 0, 0, 0);
}
...
{
	return syscall(SYS_write_pmo, cap, offset, (u64)buf, size, 0, 0, 0, 0,
		       0);
}

int usys_read_pmo(u64 cap, u64 offset, void *buf, u64 size)
{
	return syscall(SYS_read_pmo, cap, offset, (u64)buf, size, 0, 0, 0, 0,
		       0);
}
